...
def getHNPosts():
	...
	#print(data)
	for post in data:
		postResponse = requests.get('https://hacker-news.firebaseio.com/v0/item/{0}.json'.format(post))
		...
		val = (postData["id"], postData["by"], postData["title"], postData["score"], postData["url"], postData["descendants"])
...
def getDBPosts():
	...
	if flags.score.is_enabled():
		mycursor.execute("SELECT id, author, title, score, url, descendants FROM hn_posts_score")
	else:
		mycursor.execute("SELECT id, author, title, url, descendants FROM hn_posts_no_score")
